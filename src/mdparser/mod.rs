...
pub fn is_remote_url(dest: &str) -> bool {
    if dest.starts_with("//") {
        return true;
    }
...
/// Extracts the destinations of ALL image links in a markdown document,
...
/// (`![alt](img.png "title")`) are all handled per CommonMark.
///
/// # Arguments
...
pub fn extract_image_dests(content: &str) -> Vec<String> {
...
///
/// See [`extract_image_dests`] for the parsing rules; this function filters
/// out remote destinations (http/https/protocol-relative).
///
/// # Arguments
/// - `content` - Markdown file content.
///
